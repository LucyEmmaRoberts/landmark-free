...
aligned_humerus_SMA <- vector("list", length = length(meshes_humerus))

# Align each mesh NOTE SCALING IS SET TO FALSE
for (i in seq_along(meshes_humerus)) {
  aligned_mesh <- rotmesh.onto(meshes_humerus[[i]], humerus_specimen_list[[i]], humerus_specimen_list[[1]], scale = TRUE)
  aligned_humerus_SMA[[i]] <- aligned_mesh$mesh
}

aligned_humerus_SMA

# change this to where you want to meshes to end up
output_directory <- c("OUTPUT/PATH/SMA")
...
for (i in seq_along(meshes_humerus)) {
  ...
  vcgPlyWrite(aligned_humerus_SMA[[i]], output_file, format = "PLY_BINARY_LE")  # Use PLY_BINARY_BE for big-endian
}
...
# Define a list to store aligned meshes
aligned_humerus_PAIS <- vector("list", length = length(meshes_humerus))

# Align each mesh NOTE SCALING IS SET TO FALSE
for (i in seq_along(meshes_humerus)) {
  aligned_mesh <- rotmesh.onto(meshes_humerus[[i]], humerus_specimen_list[[i]], humerus_proc$consensus, scale = TRUE)
  aligned_humerus_PAIS[[i]] <- aligned_mesh$mesh
}

aligned_humerus_PAIS <- vector("list", length = length(meshes_humerus))

# change this to where you want to meshes to end up
output_directory <- c("OUTPUT/PATH/PAIS")
setwd(output_directory)
...
for (i in seq_along(meshes_humerus)) {
  ...
  output_file <- paste0(original_name, ".ply")
  # Export the mesh in binary format (choose either BE or LE)
  vcgPlyWrite(aligned_humerus_PAIS[[i]], output_file, format = "PLY_BINARY_LE")  # Use PLY_BINARY_BE for big-endian
}
...
# Define a list to store aligned meshes
aligned_humerus_PAMS <- vector("list", length = length(meshes_humerus))

# Align each mesh NOTE SCALING IS SET TO FALSE
for (i in seq_along(meshes_humerus)) {
  aligned_mesh <- rotmesh.onto(meshes_humerus[[i]], humerus_specimen_list[[i]], humerus_proc$consensus, scale = FALSE)
  aligned_humerus_PAMS[[i]] <- aligned_mesh$mesh
}
...
for (i in seq_along(meshes_humerus)) {
  scale_mesh <- scalemesh(aligned_humerus_PAMS[[i]], size = (1/cSize(aligned_humerus_PAMS[[i]]))*1000, center = "none")
  aligned_humerus_PAMS[[i]] <- scale_mesh
}

aligned_humerus_PAMS
...
for (i in seq_along(meshes_humerus)) {
  ...
  output_file <- paste0(original_name, ".ply")
  # Export the mesh in binary format (choose either BE or LE)
  vcgPlyWrite(aligned_humerus_PAMS[[i]], output_file, format = "PLY_BINARY_LE")  # Use PLY_BINARY_BE for big-endian
}
